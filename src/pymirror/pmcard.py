...
class PMCard(PMModule):
    def __init__(self, pm, config):
        ...
        self._card.header = (
            PMCardTextModule(**self._card.header.__dict__)
            if self._card.header
            else PMCardTextModule()
        )
        self._card.body = (
            PMCardTextModule(**self._card.body.__dict__)
            if self._card.body
            else PMCardTextModule()
        )
        self._card.footer = (
            PMCardTextModule(**self._card.footer.__dict__)
    ...
    def is_dirty(self) -> bool:
        return (
            self._card.header.is_dirty()
            or self._card.body.is_dirty()
            or self._card.footer.is_dirty()
        )

    def _render_text(
        self, msg, rect, card_text, maybe_invert_colors=False
    ) -> int:  # returns next y position
        ...
        lines = self.bitmap.gfx.font.text_split(msg, rect, card_text.wrap)
        self.bitmap.text_box(
            rect, lines, halign=card_text.halign, valign=card_text.valign
        )
        self.bitmap.gfx_pop()
    ...
    def render(self, force: bool = False) -> bool:
        ...
        gfx = self.bitmap.gfx
        header_height = (
            self._card.header.height or self._card.header.font_size or gfx.font_size
        )
        footer_height = (
            self._card.footer.height or self._card.footer.font_size or gfx.font_size
        )
        next_y0 = 0
        if self._card.header.text:
            next_y0 = self._render_text(
                self._card.header.text,
                (0, 0, gfx.width, header_height),
                self._card.header,
                maybe_invert_colors=True,
            )
        if self._card.footer.text:
            next_y0 = self._render_text(
                self._card.body.text,
                (0, next_y0, gfx.width, gfx.height - footer_height),
                self._card.body,
                maybe_invert_colors=False,
            )
            next_y0 = self._render_text(
                self._card.footer.text,
                (0, next_y0, gfx.width, gfx.height),
                self._card.footer,
                maybe_invert_colors=True,
            )
        else:
