...
class AnalogClockModule(PMModule):
	...
	def _render_clock_face(self, dx=0, dy=0, r=0):
		...
		gfx = self.gfx
		self.bitmap.circle(gfx, dx, dy, r, fill=gfx.bg_color)
	...
	def render(self, force: bool = False) -> bool:
		...
		if self.hour_hand is not None:
			...
			self.bitmap.line(gfx, (dx, dy, hr_posn[0], hr_posn[1]))
		...
		if self.minute_hand is not None:
			...
			self.bitmap.line(gfx, (dx, dy, min_posn[0], min_posn[1]))
		...
		if self.second_hand is not None:
			...
			self.bitmap.line(gfx, (dx, dy, sec_posn[0], sec_posn[1]))
