...
class PMCard(PMModule):
	...
	def _render_text(self, msg, rect, font_info, invert_colors=False) -> int: # returns next y position
		gfx = self.gfx
		gfx2 = copy.copy(self.gfx)
		gfx2.set_font(font_info.font or gfx.font_name, font_info.font_size or gfx.font_size)
		gfx2.text_color = font_info.color or gfx.text_color if not invert_colors else gfx.text_bg_color
		gfx2.text_bg_color = font_info.bg_color or gfx.text_bg_color if not invert_colors else gfx.text_color
	...
	def render(self, force: bool = False) -> bool:
		...
		self._render_text(self.body, (gfx.x0, gfx.y0 + header_height, gfx.x1, gfx.y1 - footer_height), self.config.card.body, invert_colors=True)
