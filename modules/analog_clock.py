...
class AnalogClock(PMModule):
	...
	def _render_clock_face(self, dx=0, dy=0, r=0):
		...
		for posn in _compute_clock_positions(gfx.x0 + dx, gfx.y0 + dy, r-gfx.font_height):
			self.screen.text_box(gfx, str(hr), (posn[0], posn[1], posn[0]+gfx.font_width*len(str(hr)), posn[1]+gfx.font_height), valign="center", halign="center")
			hr += 1
		gfx.line_width = 3
